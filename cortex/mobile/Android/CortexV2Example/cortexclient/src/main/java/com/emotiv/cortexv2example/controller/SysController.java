        ...
        try {
            ...
            if (mSysInterface != null) {
                if (sysJson.has("sys")) {
                    ...
                    CortexResponseController.getInstance().getDataStream().getmTrainingObject().setCurrentDetection(sysContentJson.get(0).toString());
                    CortexResponseController.getInstance().getDataStream().getmTrainingObject().setCurrentTrainingEvent(sysContentJson.get(1).toString());

                    if (sysContentJson.get(1).toString().contains("Started")) {
                        mSysInterface.onTrainingStarted(sysContentJson.get(0).toString(), CortexResponseController.getInstance().getDataStream().getmTrainingObject().getTrainingAction(), sysContentJson.get(1).toString());
